...
import { flattenAttributes } from '@pathloom/core'
...
```sh
cd dist/binaries/pathloom-studio-<platform>
PORT=4318 ./pathloom-studio
```
...
Important: keep `pathloom-studio[.exe]`, `build/`, and `proto/` in the same folder when testing or distributing executables.
