# Implementation Plan: pathloom-studio

A lightweight, local OpenTelemetry trace viewer inspired by Honeycomb's trace detail UI.
`pathloom-studio` is a **local-first debugging tool** for OpenTelemetry during development.
...
`pathloom-studio` OSS executable distribution uses Node 22 SEA (Single Executable Applications).
...
- `DOCKERHUB_USERNAME`
- `DOCKERHUB_TOKEN`
- `DOCKERHUB_REPOSITORY` (for example, `jay/pathloom-studio`)
