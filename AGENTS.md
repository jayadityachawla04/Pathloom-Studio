# pathloom-studio Agent Instructions
...
Use `flattenAttributes()` from `@pathloom/core` ([packages/core/src/attributes.ts](packages/core/src/attributes.ts)) — handles all 7 AnyValue variants.
...
- `PATHLOOM_MAX_TRACES`: max traces in memory (integer 1-10 000, default 1000)
- `PATHLOOM_MAX_LOGS`: max logs in memory (integer 1-10 000, default 1000)
- `PATHLOOM_PERSISTENCE_MODE`: `memory` or `pglite` (invalid values warn and fall back to `memory`)
- `PATHLOOM_PERSISTENCE_PATH`: local PGlite data path (default `\.pathloom/pglite`)
- `PATHLOOM_PERSISTENCE_FLUSH_MS`: flush debounce in ms (50-60000, default `750`)
- `PATHLOOM_PERSISTENCE_BACKEND_MODULE`: optional module id/path dynamically imported to register extra backends
...
External backend interop detail: before dynamically importing backend modules, OSS hydrates selected license-related env vars from `$env/dynamic/private` into `process.env` (`PATHLOOM_LICENSE_*`) so external modules reading `process.env` still work.
...
| File                                                                              | What's covered                                                                                                                       |
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| [attributes.test.ts](packages/core/src/attributes.test.ts)                        | All 7 AnyValue variants, null/edge cases (canonical tests in `@pathloom/core`)                                                       |
...
| [traceStore.test.ts](src/lib/server/traceStore.test.ts)                           | Ingestion, span merging, FIFO eviction, subscribe/unsubscribe, selected trace deletion (`resolveRootSpanName` from `@pathloom/core`) |
...
5. **Configurable retention** — `PATHLOOM_MAX_TRACES` env var (integer 1-10 000, default 1000). FIFO eviction. Read via `$env/dynamic/private` in `traceStore.ts`. Exposed as `traceStore.maxTraces`. Invalid values warn and fall back to 1000. Logs have an independent `PATHLOOM_MAX_LOGS` env var (same range/defaults), exposed as `traceStore.maxLogs`.
6. **Persistence is optional** — OSS always has `memory`; `pglite` requires a registered backend (usually via `PATHLOOM_PERSISTENCE_BACKEND_MODULE`). If unavailable, runtime falls back to memory with `persistence.unavailableReason` in `GET /api/config`.
7. **Update availability check** — On mount, the page fetches `https://api.github.com/repos/jay/pathloom-studio/releases/latest` (GitHub API) and compares `tag_name` to `import.meta.env.PACKAGE_VERSION` (embedded at build time from `package.json` via Vite `define`). If a newer semver is found, a non-intrusive notice is shown in the bottom-left of the trace list, next to the retention notice. The notice links to `https://github.com/jay/pathloom-studio/releases` and can be dismissed per-version via `localStorage`. The result is cached in `localStorage` (`update-check-cache`) for 1 hour to avoid hitting the GitHub rate limit (60 unauthenticated req/h). Network failures are silent and do not affect the UI.
