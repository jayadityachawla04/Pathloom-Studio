...
// origin(s) are configured via PATHLOOM_CORS_ALLOWED_ORIGINS (default: any origin).
export const handle: Handle = async ({ event, resolve }) => {
  ...
  if (corsEnabled && request.method === 'OPTIONS') {
    return new Response(null, {
      status: 204,
      headers: buildCorsHeaders(request, env.PATHLOOM_CORS_ALLOWED_ORIGINS),
    })
  }
  ...
  if (corsEnabled) {
    const corsHeaders = buildCorsHeaders(
      request,
      env.PATHLOOM_CORS_ALLOWED_ORIGINS,
    )
