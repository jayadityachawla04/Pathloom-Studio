...
function resolveMaxTraces(): number {
  const raw = env.PATHLOOM_MAX_TRACES
  if (raw === undefined || raw === '') return 1000
  const parsed = Number.parseInt(raw, 10)
  if (Number.isNaN(parsed) || parsed < 1 || parsed > 10_000) {
    console.warn(
      `[pathloom-studio] Invalid PATHLOOM_MAX_TRACES="${raw}". Must be an integer between 1 and 10000. Falling back to 1000.`,
    )
    return 1000
  }
  return parsed
...
function resolveMaxLogs(): number {
  const raw = env.PATHLOOM_MAX_LOGS
  if (raw === undefined || raw === '') return 1000
  const parsed = Number.parseInt(raw, 10)
  if (Number.isNaN(parsed) || parsed < 1 || parsed > 10_000) {
    console.warn(
      `[pathloom-studio] Invalid PATHLOOM_MAX_LOGS="${raw}". Must be an integer between 1 and 10000. Falling back to 1000.`,
    )
    return 1000
  }
  return parsed
...
function resolvePersistenceMode(): 'memory' | 'pglite' {
  const raw = env.PATHLOOM_PERSISTENCE_MODE
  if (raw === undefined || raw === '') return 'memory'
  if (raw === 'memory' || raw === 'pglite') return raw
  console.warn(
    `[pathloom-studio] Invalid PATHLOOM_PERSISTENCE_MODE="${raw}". Must be "memory" or "pglite". Falling back to "memory".`,
  )
  return 'memory'
}

function resolvePersistencePath(): string {
  const raw = env.PATHLOOM_PERSISTENCE_PATH
  if (raw === undefined || raw === '') return '\.pathloom/pglite'
  return raw
}

function resolveFlushMs(): number {
  const raw = env.PATHLOOM_PERSISTENCE_FLUSH_MS
  if (raw === undefined || raw === '') return 750
  const parsed = Number.parseInt(raw, 10)
  if (Number.isNaN(parsed) || parsed < 50 || parsed > 60_000) {
    console.warn(
      `[pathloom-studio] Invalid PATHLOOM_PERSISTENCE_FLUSH_MS="${raw}". Must be an integer between 50 and 60000. Falling back to 750.`,
    )
    return 750
  }
  return parsed
...
function resolvePersistenceBackendModule(): string | null {
  const raw = env.PATHLOOM_PERSISTENCE_BACKEND_MODULE
...
function hydrateProcessEnvForExternalBackends(): void {
  ...
  const mappings: Array<[key: string, value: string | undefined]> = [
    ['PATHLOOM_LICENSE_KEY', env.PATHLOOM_LICENSE_KEY],
    ['PATHLOOM_LICENSE_PUBLIC_KEY_PEM', env.PATHLOOM_LICENSE_PUBLIC_KEY_PEM],
    ['PATHLOOM_LICENSE_PUBLIC_KEY_PATH', env.PATHLOOM_LICENSE_PUBLIC_KEY_PATH],
    ['PATHLOOM_LICENSE_CLOCK_SKEW_SEC', env.PATHLOOM_LICENSE_CLOCK_SKEW_SEC],
  ]
...
async function loadExternalBackends(): Promise<void> {
  ...
  try {
    ...
    console.warn(
      `[pathloom-studio] Persistence backend module "${persistenceBackendModule}" loaded but did not export registerTraceStoreBackends().`,
    )
  } catch (error) {
    externalBackendLoadError = 'backend-module-load-failed'
    if (isMissingBackendModuleError(error)) {
      console.info(
        `[pathloom-studio] Optional persistence backend module "${persistenceBackendModule}" was not found. Using built-in backends (memory mode in OSS).`,
      )
      return
    }

    console.error(
      `[pathloom-studio] Failed to load persistence backend module "${persistenceBackendModule}". Falling back to built-in backends.`,
      error,
    )
  }
}
...
async function createTraceStore(): Promise<TraceStoreWithPersistenceStatus> {
  ...
  if (backend) {
    try {
      ...
      if (isExpectedBackendInitFallback(error)) {
        console.info(
          `[pathloom-studio] ${persistenceMode} persistence is unavailable (${backendInitError}); using in-memory mode.`,
        )
      } else {
        console.error(
          `[pathloom-studio] Failed to initialize ${persistenceMode} persistence, using in-memory mode instead.`,
          error,
        )
      }
    }
    ...
    console.warn(
      `[pathloom-studio] Persistence backend "${persistenceMode}" is not available in this build. Falling back to in-memory mode.`,
    )
  }
  ...
  if (!memoryBackend) {
    throw new Error(
      '[pathloom-studio] Memory trace store backend is not registered.',
    )
  }
