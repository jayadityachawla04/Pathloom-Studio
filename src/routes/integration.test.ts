...
describe('GET /api/traces/:traceId/export', () => {
  it('exports a trace as a downloadable pathloom-studio JSON envelope', async () => {
    await POST({ request: makePostRequest(simpleTrace) } as any)
    ...
    expect(payload.format).toBe('pathloom-session-export')
...
describe('POST /api/traces/import/preview', () => {
  it('previews pathloom-studio export metadata before import', async () => {
    await POST({ request: makePostRequest(simpleTrace) } as any)
    ...
    expect(preview.format).toBe('pathloom-session-export')
  ...
  it('uses envelope traceCount metadata in preview', async () => {
    const syntheticEnvelope = {
      format: 'pathloom-session-export',
      version: 1,
